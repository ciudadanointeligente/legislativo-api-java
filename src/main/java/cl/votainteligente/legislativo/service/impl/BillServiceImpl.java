...
public class BillServiceImpl extends EntityManagerService implements BillService {
	...
	@Override
	public Page<BillDO> getAllBillDOs(int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("select p from Bill p");
		...
		Query queryCount = getEntityManager().createQuery("select count(p) from Bill p");
	...
	@Override
	public Page<BillDO> getByDateRange(Date from, Date to, int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("select p from Bill p where p.entryDate >= ? and p.entryDate <= ?");
		...
		Query queryCount = getEntityManager().createQuery("select count(p) from Bill p where p.entryDate >= ? and p.entryDate <= ?");
	...
	@Override
	public Page<BillDO> getByStage(StageDescription stageDescription, int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("select distinct p from Bill p join p.stages s where s.endDate is null and s.stageDescription = ?");
		...
		Query queryCount = getEntityManager().createQuery("select count(distinct p) from Bill p join p.stages s where s.endDate is null and s.stageDescription = ?");
	...
	public Page<BillDO> getByAuthor(Person person, int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("select p from Bill p join p.authors r where r = ?");
		...
		Query queryCount = getEntityManager().createQuery("select count(p) from Bill p join p.authors r where r = ?");
	...
	public Page<BillDO> getByMatter(Matter matter, int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("select p from Bill p where p.matter = ?");
		...
		Query queryCount = getEntityManager().createQuery("select count(p) from Bill p where p.matter = ?");
	...
	@Override
	public Page<BillRoleDO> getByAuthorRole(Person person, int pageNumber, int resultsPerPage) throws ServiceException {
		Query query = getEntityManager().createQuery("SELECT b,r FROM Bill b JOIN b.authors a JOIN a.roles r  " +
														"WHERE a = ? AND " +
